...
import static org.firstinspires.ftc.teamcode.teamcode.Subsystems.MainIntake.IntakeStates.SEARCHBOTH;
...
@Autonomous(name = "BucketAutoRed")
public class BucketAutoRed extends BaseOpMode {
    ...
    public void goToScore(){
        ...
        if (timeWaste.seconds() > .456 && done4 && cycles == 2){
            intake.setState(HOMEUP);
            scoring.setState(TRANSFER);
            done4 = false;
    ...
    public void submersible(){
        ...
        if (timeWaste.seconds() < 1.2) {
            ...
            drive.holdPosition(67 , 92,0.2, .7);
            holdY = 92;
        ...
        if ((timeWaste.seconds()>2 && (drive.getVelocity()[1] > -0.5)&& holdY == 92) || (timeWaste.seconds() > 2.5 && holdY == 92)) {
            holdY = drive.getY() - 1;
            intake.sweeperOut();
            intake.setState(SEARCHBOTH);


            done1 = false;
        }
        if (intake.getState() == SEARCHBOTH) {
