...
@Autonomous(name = "REDCLOSEBucketAuto")
@Disabled
public class RedCloseBucketAuto extends BaseOpMode {
    ...
    public void Score1() {
        ...
        if (timeWaste.seconds() > 1.7 && done1){
            scoring.setState(HIGHBUCKETSCORE);
            done1 = false;
        } else if(scoring.getState() == HOME && timeWaste.seconds() > 1.7){
            setState(FarSpike);
        }
    }
    ...
    public void score2() {
        drive.holdPosition(4.7, 119.45, 1.43, .6);
    ...
    public void goToScore(){
        ...
        if (timeWaste.seconds() > 1 && scoring.getState() == POSTTRANSFER)
            setState(Score2);
    ...
    public void submersible(){
        ...
        if (timeWaste.seconds() < 2) {
            drive.holdPosition(64, 110, 0, 1);
        } else {
            drive.holdPosition(64, 92,0, .6);
        }
