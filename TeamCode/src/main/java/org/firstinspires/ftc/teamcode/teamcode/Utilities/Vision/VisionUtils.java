...
public class VisionUtils {
    ...
    public static List<RotatedRect> sortRotatedRectsByMaxOption(int n, RECT_OPTION option, List<RotatedRect> rects){
        List<RotatedRect> sorted_rects = new ArrayList<>();
        for (int j=0; j < n; j++){
            int alpha_i = getMaxIndex2(rects, option);
            sorted_rects.add(rects.get(alpha_i));
    ...
    public static int getMaxIndex2(List<RotatedRect> rects, RECT_OPTION option){
        int alpha_index = 0;
        double max = Integer.MIN_VALUE;
        double cur = 0;
        for (int i=0; i < rects.size(); i++){
            ...
            switch (option){
                case X:
                    cur = rects.get(i).boundingRect().x;
                    break;

                case Y:
                    cur = rects.get(i).boundingRect().y;
                    break;

                case WIDTH:
                    cur = rects.get(i).boundingRect().width;
                    break;

                case HEIGHT:
                    cur = rects.get(i).boundingRect().height;
                    break;

                case AREA:
                    cur = rects.get(i).boundingRect().width * rects.get(i).boundingRect().height;
                    break;
